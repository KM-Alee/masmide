...
pub fn render(frame: &mut Frame, area: Rect, theme: &Theme, scroll: usize) {
    ...
    let key_style = Style::default().fg(theme.ui.title_focused.to_color()).add_modifier(Modifier::BOLD);
    let desc_style = Style::default().fg(theme.ui.foreground.to_color());
    let section_style = Style::default().fg(theme.syntax.keyword.to_color()).add_modifier(Modifier::BOLD);
    ...
    let title = Line::from(vec![
        ...
        Span::styled("MASMIDE", Style::default().fg(theme.ui.title_focused.to_color()).add_modifier(Modifier::BOLD)),
        Span::styled(" Help ", Style::default().fg(theme.ui.foreground.to_color())),
    ]);
...
pub fn total_lines() -> usize {
    ...
    for (_, bindings) in HELP_SECTIONS {
        ...
        count += (bindings.len() + 1) / 2; // Bindings (2 per row)
        count += 1; // Empty line
    }
    count
}
